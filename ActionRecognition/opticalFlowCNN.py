...
# test set
test_files = [f'TV-HI/test/{action_categories[c]}_{i:04d}-8.png' for c in range(len(action_categories)) for i in set_1_indices[c]]
test_labels = [f'{action_categories[c]}' for c in range(len(action_categories)) for i in set_1_indices[c]]

# training set
train_files = [f'TV-HI/train/{action_categories[c]}_{i:04d}-8.png' for c in range(len(action_categories)) for i in set_2_indices[c]]
...
model = Sequential()
model.add(Conv2D(32, kernel_size = (5, 5), activation = 'relu', input_shape = (112, 112, 3)))
...
history = model.fit(np.array(X_train_images), train_labels, batch_size = 128, epochs = 70, verbose = 2)
