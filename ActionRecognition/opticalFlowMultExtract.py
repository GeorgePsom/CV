...
for i in range(0,len(extract_set)):
    video_no = i  # change this to a number between [0, 100] and you can see a different training video from Set 2

    cap = cv2.VideoCapture(f'TV-HI/tv_human_interactions_videos/{extract_set[video_no]}')
    ...
    while(1):
        ...
        if counter % step_frame == 1 or step_frame == 1:
            cv2.imwrite('TV-HI/test/'+ extract_set[video_no][0:-4] + '-' +  str(step_count) +  '.png', rgb)
            step_count += 1

        #if mid_frame == counter:
        #    cv2.imwrite('TV-HI/midframesTrain/'+ extract_set[video_no][0:-4] +  '.png', frame2)
        #    step_count += 1
